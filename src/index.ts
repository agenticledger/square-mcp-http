#!/usr/bin/env node
/**
 * Square MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Square API key as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Square payments, orders, invoices, customers, catalog, inventory, and more through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Square Access Token as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-square-access-token>',
      howToGetKey:
        'Generate an access token at https://developer.squareup.com/apps → OAuth or create a sandbox token.',
    },
    configTemplate: {
      mcpServers: {
        square: {
    ...
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/square/',
    },
  });
});
...
<head>
  ...
  <title>Square MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Square MCP Server</h1>
    <p class="subtitle">Access Square payments, orders, invoices, customers, catalog, inventory, and more through MCP tools.</p>

    <div class="card">
      <h2>How to get your Square Access Token</h2>
      <ol class="steps">
        <li>Go to the <a href="https://developer.squareup.com/apps" target="_blank" rel="noopener">Square Developer Dashboard</a>.</li>
        <li>Select or create an application.</li>
        <li>Navigate to <strong>Credentials</strong>.</li>
        <li>Copy your <strong>Access Token</strong> (sandbox or production).</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">Square Access Token</label>
...
function createMCPServer(client: SquareClient): Server {
  const server = new Server(
    { name: 'square-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  const client = new SquareClient(apiKey);
...
<head>
  ...
  <title>Authorize \u2014 Square MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Square MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Square MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Square MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides Square access token)`);
});
