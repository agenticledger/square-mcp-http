#!/usr/bin/env node
/**
 * Square MCP Server — Streamable HTTP, BROKER-FIRST (auth model "B").
 *
 * This MCP holds ZERO Square secrets. It is a *client* of the Connections Broker
 ...
 * POST /token for provider=square -> { accessToken }, call the Square API
...
app.get('/', (_req, res) => {
  res.json({
    ...
    version: '2.0.0',
    description: 'Manage Square payments, orders, customers, catalog, and inventory via MCP.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      ...
      alternativeAuth: {
        ...
        description: 'Escape hatch (no secret held): pass a raw Square access token as Bearer.',
      },
    },
    configTemplate: {
      mcpServers: {
...
app.get('/health', (_req, res) => {
  res.json({
    ...
    authModes: [
      'broker-first (default): resolves Square via the Connections Broker',
      'bearer-passthrough (escape hatch): Authorization: Bearer <square-access-token>',
    ],
  });
});
...
type ClientResolution =
  | { kind: 'client'; client: SquareClient }
...
function createMCPServer(resolveClient: ClientResolver): Server {
  const server = new Server(
    { name: 'square-mcp-server', version: '2.0.0' },
    { capabilities: { tools: {} } }
  );
...
async function connectMessage(principal: string): Promise<string> {
    ...
    return JSON.stringify(
      {
        ...
        authKind: 'static',
        message:
          'Square is not connected for this caller yet. This is an API-key provider: an admin must vault the key with the Connections Broker (POST /credential for this provider). Once vaulted, run the tool again — it will work.',
      },
      null,
      2
    );
  ...
  if ('error' in started) {
    return `Square isn't connected for this caller yet, and starting a connection failed: ${started.error}`;
  }
  return JSON.stringify(
    {
      status: 'connection_required',
      provider: brokerProvider,
      message:
        'Square is not connected for this caller yet. Open the connect link below once (sign in and grant access), then run the tool again — it will work.',
...
app.post('/mcp', async (req, res) => {
  ...
  if (raw) {
    const client = new SquareClient(raw);
    ...
    if (!brokerConfigured) {
      res.status(503).json({
        ...
        alternative: { 'Authorization': 'Bearer <your-square-access-token>' },
      });
      return;
    }
    ...
    resolveClient = async () => {
      ...
      if (tok.status === 'connected') {
        return { kind: 'client', client: new SquareClient(tok.accessToken) };
      }
...
app.listen(PORT, () => {
  console.log(`Square MCP HTTP Server v2.0.0 (broker-first)`);
